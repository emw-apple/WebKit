...
class TestIdentifierEncoding(unittest.TestCase):
    ...
    def _encode(self, ident: str) -> str:
        m = Mangler()
        m._mangle_identifier(ident)
        return "".join(m.buffer)

    def test_simple_word(self):
        self.assertEqual(self._encode("foo"), "3foo")

    def test_single_char(self):
        self.assertEqual(self._encode("x"), "1x")

    def test_camel_case_splits(self):
        # "ModuleName" -> words: Module, Name -> no prior words -> just literal
        self.assertEqual(self._encode("ModuleName"), "10ModuleName")
    ...
    def test_underscore_prefix(self):
        ...
        m = Mangler()
        m._mangle_identifier("_arg")
        result = "".join(m.buffer)
        self.assertEqual(result, "4_arg")

    def test_digit_first_literal(self):
        # If first char of a literal substring is a digit, emit 'X' instead
        # This is for error recovery
        m = Mangler()
        m._mangle_identifier("a1b")
        # "a" not word (len < 2), "1b" not word start (digit)
        # Actually: word scan: a=word start, 1=word end -> word "a" (len 1, too short)
        # Then 1 is not word start (digit), b is word start, \0 is word end -> word "b" (len 1, too short)
